...
// Topic A simple string used to describe a topic
type Topic = string
...
// Subscription A handle to a bus subscription
...
// NewBus Create a new event bus with a default table size of 512.
func NewBus() *Bus {
	return NewSizedBus(512)
}

// NewSizedBus Create a new event bus with a specific hash table size
...
// Subscribed Test if this subscription is currently subscribed
func (s *Subscription) Subscribed() bool {
	return !decodeFlags(a.LoadUintptr(&s.flags)).unsubscribed
}

// CallbackArity Get the number of arguments of the callback attached to this subscription.
...
// Lock copying will trigger go vet copy locks warning
func (_ *noCopy) Lock()   {}
